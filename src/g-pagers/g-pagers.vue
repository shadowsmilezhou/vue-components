<template>
  <div class="g-pager" :class="{hide: hideIfOnePage === true && totalPage <= 1}">
    <span class="g-pager-nav prev" :class="{disabled:currentPage===1}"
      ...
      <g-icon name="backward" :fill="color"></g-icon>
    </span>
    ...
    <span class="g-pager-nav next" :class="{disabled: currentPage===totalPage}"
          @click="onClickPage(currentPage+1)">
      <g-icon name="forward" :fill="color"></g-icon>
    </span>
  </div>
</template>
<script>
...
<style scoped lang="scss">
  ...
  .g-pager { display: flex; justify-content: flex-start; align-items: center; user-select: none;
    ...
    &-nav {
      ...
      cursor: pointer;color: $bgc-color;
      &.disabled {
        cursor: default;
